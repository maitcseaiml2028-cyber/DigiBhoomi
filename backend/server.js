...
app.post('/api/documents', requireAuth, (req, res) => {
  ...
  db.prepare(`
    ...
    VALUES (?, ?, ?, ?, ?, 'Pending', ?, NULL, ?)
  `).run(id, user.id, projectId || 'DL-001', parcelId || 'DLP-00043', type || 'Supporting Document', uploadedAt, description || '');

  auditLog(user.name, 'Document uploaded', id, `${type} · ${id}`);
  res.status(201).json({ id, type, status: 'Pending', uploadedAt, description });
});
