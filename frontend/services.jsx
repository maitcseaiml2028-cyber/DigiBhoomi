...
const predictionService = {
  getRiskPrediction(projectId){
    let p = projectService.getProject(projectId);
    // Fall back to the user's first accessible project rather than returning
    // null, so pages never crash if the requested/URL project id is out of
    // this user's scope (e.g. a stale link or an out-of-scope default).
    if(!p) p = (window.BS_DATA.PROJECTS || [])[0];
    if(!p) return null;
    return {
